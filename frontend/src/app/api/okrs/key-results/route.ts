...
import {
  ...
  updateAgileKeyResultInBackend,
  type CreateAgileKeyResultInput,
  type UpdateAgileKeyResultInput
} from "@/lib/okrs";
...
export async function PATCH(request: Request) {
  ...
  try {
    const body = (await request.json()) as UpdateAgileKeyResultInput;
    const projectId = body.projectId?.trim();
    const objectiveId = body.objectiveId?.trim();
    ...
    if (!canWriteProject) {
      return NextResponse.json({ message: "No autorizado para editar en este proyecto." }, { status: 403 });
    }
    ...
    if (objectiveId) {
      ...
      if (!canWriteObjective) {
        return NextResponse.json(
          { message: "No autorizado para mover Key Results a este Objective." },
          { status: 403 }
        );
      }
    }

    const payload = await updateAgileKeyResultInBackend({
      ...body,
      objectiveId,
      projectId,
      recordId
