...
import { fetchAgileKeyResultHistoryBulkFromBackend } from "@/lib/okrs";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const payload = await fetchAgileKeyResultHistoryBulkFromBackend(keyResultIds);
    ...
    const message =
      error instanceof Error
        ? error.message
        : "No fue posible consultar el histórico de Key Results.";
