...
export async function GET(request: Request) {
  ...
  try {
    ...
    const payload = await fetchAgileObjectivesFromBackend(projectId);
    ...
    const message =
      error instanceof Error
        ? error.message
        : "No fue posible consultar los objetivos de OKRs.";
...
export async function POST(request: Request) {
  ...
  try {
    const body = (await request.json()) as CreateAgileObjectiveInput;
    ...
    const payload = await createAgileObjectiveInBackend({
      ...body,
      projectId
    });
