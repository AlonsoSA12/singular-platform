...
import {
  createSingularAgileObjectiveInBackend,
  fetchAgileProjectsFromBackend,
  updateSingularAgileObjectiveInBackend,
  type SingularAgileObjectiveInput,
  type SingularAgileObjectiveUpdateInput
} from "@/lib/okrs";
...
function getProjectIdFromBody(body: Partial<SingularAgileObjectiveInput>) {
...
export async function POST(request: Request) {
  ...
  try {
    const body = (await request.json()) as SingularAgileObjectiveInput;
    ...
    const payload = await createSingularAgileObjectiveInBackend(body);

    return NextResponse.json(payload, { status: 201 });
  } catch (error) {
    const message =
      error instanceof Error ? error.message : "No fue posible crear el Objective en Singular Agile.";
...
export async function PATCH(request: Request) {
  ...
  try {
    const body = (await request.json()) as SingularAgileObjectiveUpdateInput;
    const projectId = getProjectIdFromBody(body);
    ...
    const payload = await updateSingularAgileObjectiveInBackend(body);

    return NextResponse.json(payload);
  } catch (error) {
    const message =
      error instanceof Error ? error.message : "No fue posible actualizar el Objective en Singular Agile.";
